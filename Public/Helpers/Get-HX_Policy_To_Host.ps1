...
.DESCRIPTION
   Long description
.EXAMPLE
   $x = Get-HX_Policy_To_Host
.EXAMPLE
   $x|?{$_.Children.values.hostname -contains "THEHOSTYOUSEEK"}
.INPUTS
...
#>
function Get-HX_Policy_To_Host{
	...
	foreach ($policy in $target){
		...
		$policy.psobject.properties.add([psnoteproperty]::new("host_count",$hostcount))

		#if going full out go ahead and strap these kids down
		if (!$measure){
			$policy.psobject.properties.add([psnoteproperty]::new("hostsets",($ht_temp.keys|%{[pscustomobject]@{name=$_;children=$ht_temp.$_}})))
		}

		Write-Host "Total hosts from selected policies: $(($target.hostcount|Measure-Object -Sum).sum)"

		Clear-Variable hostcount, ht_temp -ErrorAction SilentlyContinue
	}
	$target
